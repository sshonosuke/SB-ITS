######  Binary response  ######
# Y: binary response variable (1 or 0) 
# X: biomarkers 
# Z: adjutment covariate
# PS: propensity score
# Tr: treatment indicator (1 or -1)
# L: number of knots 

HTE.ODP.bin=function(Y, X, Z, Tr, PS=NULL, L=100, alpha.set=NULL, approx=F, maxitr=100){
  ...
  for(k in 1:p){
    u1 <- Tr*X[,k]
    u2 <- Tr*Z
    if( is.null(Z)==F ){ fit <- glm(Y~-1+Tr+u1+u2, family="binomial", weights=1/ww) }
    if( is.null(Z)==T ){ fit <- glm(Y~-1+Tr+u1, family="binomial", weights=1/ww) }
    Est[k,] <- summary(fit)$coefficients[2,1:2]
  ...
  if(approx==F){
    tt <- apply(Y/ww*Tr*X, 2, sum)
    if( is.null(Z)==F ){ mat <- Tr*cbind(1, Z) }
    if( is.null(Z)==T ){ mat <- Tr*rep(1,n) }
    ad <- mat%*%t(NP)
    XT <- Tr*X
    PL <- function(beta){
      val <- apply(log(1+exp(ad+XT*beta))/ww, 2, sum)
      return( tt*beta-val )
    }
